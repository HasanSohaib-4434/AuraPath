  ...
  <div>
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-10 text-center">
      <motion.div
        ...
        className="mb-4 inline-flex items-center gap-2 rounded-full border border-aura-500/30 bg-aura-500/10 px-4 py-1.5 text-xs font-medium text-aura-300"
      >
      ...
      >
        ...
        <br />
        <span className="text-zinc-100">Your pace.</span>
      </motion.h1>
      <motion.p
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        {STEPS.map(({ icon: Icon, label, desc }, i) => (
          <motion.button
            ...
            className="glass-card-interactive group flex flex-col items-center p-4 text-center"
          >
            <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-surface-elevated text-aura-400 transition group-hover:bg-aura-500/15 group-hover:text-aura-300">
    ...
    {compareVariants?.length ? (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto mt-8 max-w-3xl">
        <h3 className="mb-4 text-center text-sm font-semibold text-zinc-300">Pick a path variant</h3>
        <div className="grid gap-4 sm:grid-cols-2">
          {compareVariants.map((v) => (
            <motion.button
              ...
              className="glass-card-interactive p-5 text-left"
            >
              <span className="text-xs uppercase text-aura-400">{v.variant || 'path'}</span>
              <div className="mt-1 font-semibold text-zinc-100">{v.title}</div>
              <p className="mt-2 line-clamp-2 text-xs text-zinc-500">{v.description}</p>
              <p className="mt-2 text-xs text-zinc-600">{v.levels?.length || 0} levels</p>
            </motion.button>
          ))}
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto mt-8 max-w-md text-center">
        <div className="glass-card p-6">
          <div className="mx-auto mb-3 h-1.5 w-full overflow-hidden rounded-full bg-surface-elevated">
            <motion.div
              className="h-full rounded-full bg-gradient-to-r from-aura-600 via-cyan-400 to-emerald-400"
              initial={{ width: '0%' }}
              animate={{ width: '90%' }}
              transition={{ duration: 10, ease: 'easeInOut' }}
            />
          </div>
          <motion.p key={loadingTip} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="text-sm text-zinc-400">
