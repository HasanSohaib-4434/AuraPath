...
const StudyHub = ({
  ...
  if (!roadmap) {
    return (
      <div className="glass-card py-20 text-center">
        <MessageCircle className="mx-auto mb-3 h-10 w-10 text-zinc-600" />
        <p className="text-zinc-400">Open or create a learning path first.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-zinc-100 sm:text-3xl">Study Hub</h1>
        <p className="mt-2 text-sm text-zinc-400">
          Chat with your PDFs, flashcards, and focus timer for <span className="text-aura-300">{roadmap.title}</span>
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,280px)_1fr]">
        ...
        >
          <h2 className="text-sm font-semibold text-zinc-200">Study material</h2>
          {pdfReady ? (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4 text-sm">
              <div className="font-medium text-emerald-300">PDF indexed</div>
              <div className="mt-1 truncate text-xs text-zinc-500">{pdfFilename}</div>
              <div className="mt-2 text-xs text-zinc-400">{pdfChunkCount} searchable sections</div>
              {pdfList.length > 1 ? (
            ...
            <p className="text-xs text-zinc-500">Upload PDFs to enable chat and flashcards from material.</p>
          )}
          <motion.button
            type="button"
            whileTap={{ scale: 0.98 }}
          ...
          <div className="flex items-center justify-between border-t border-surface-border pt-4">
            <span className="text-xs text-zinc-500">Show sources</span>
            <button
              type="button"
              role="switch"
        ...
        >
          <div ref={scrollRef} className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
            {!pdfReady ? (
              ...
              <div className="py-6 text-center">
                <Bot className="mx-auto mb-2 h-8 w-8 text-aura-400" />
                <p className="mb-4 text-sm text-zinc-400">Try a quick prompt:</p>
              ...
              >
                ...
                >
                  ...
                  {m.role === 'assistant' && m.content && !m.streaming ? (
                    <ListenButton text={m.content} messageKey={`msg-${i}`} className="text-zinc-500 hover:text-aura-300" />
                  ) : null}
                </div>
              </motion.div>
            ))}
          ...
          {showSources && latestSources.length ? (
            <div className="max-h-32 shrink-0 overflow-y-auto border-t border-surface-border bg-surface/50 px-4 py-2">
              <div className="mb-1 text-xs font-medium text-zinc-500">Sources</div>
              {latestSources.map((s, j) => (
                <div key={j} className="mb-2 text-xs text-zinc-500 line-clamp-2">
                  {s.text}
                </div>
              ))}
            </div>
          ...
          >
            <div className="flex gap-2">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="inline-flex shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-aura-600 to-aura-500 px-3.5 py-2.5 text-white disabled:opacity-50"
              >
